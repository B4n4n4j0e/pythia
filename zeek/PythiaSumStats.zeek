...
event connection_state_remove(c: connection) &priority = 0
    ...
    SumStats::observe("prot",SumStats::Key($str=cat(c$conn$proto)),SumStats::Observation($num=1));
...
event zeek_init() {
    ...
    local rProt = SumStats::Reducer($stream="prot",$apply=set(SumStats::SUM));
    ...
    local rRespPOI = SumStats::Reducer($stream="resp_poi",$apply=set(SumStats::SUM));
        ...
        SumStats::create([	$name = "top k dns queries",
                    ...
                    s = topk_get_top(r$topk,10);
                		...
                		Log::write(DnsTopK::LOG, [$ts=ts, $query=query,$counter=topk_count(r$topk,s[i])]);
        	...
        	SumStats::create([	$name = "top k resp host",
                        ...
                        Log::write(RespHostTopK::LOG, [$ts=ts, $host=host,$counter=topk_count(r$topk,s[i])]);
       	...
       	SumStats::create([	$name = "top k origin host",
                        ...
                        Log::write(OriginHostTopK::LOG, [$ts=ts, $host=host,$counter=topk_count(r$topk,s[i])]);
	...
	SumStats::create([	$name = "top k resp port",
						...
						s = topk_get_top(r$topk,10);
                            ...
                            if ( i == 10 )
                                break;
							local prot : string = split_string(s[i]$str,/\//)[1];
							local p: port = to_port(s[i]$str);
                            Log::write(RespPortTopK::LOG, [$ts=ts, $p=p,$prot=prot, $counter=topk_count(r$topk,s[i])]);
                            }
	...
	SumStats::create([	$name = "sum protocol types",
						...
						$epoch_result(ts:time, key:SumStats::Key, result: SumStats::Result) = {
						local r = result["prot"];
			            Log::write(ProtSum::LOG, [$ts=ts, $prot=key$str ,$counter=r$num]);
						}]);
		...
		SumStats::create([	$name = "sum service types",
			            ...
			            Log::write(ServiceSum::LOG, [$ts=ts, $service=key$str ,$counter=r$num]);
							}]);
		...
		SumStats::create([	$name = "sum poi ",
						...
						local prot : string = split_string(key$str,/\//)[1];
						local p: port = to_port(key$str);
			            Log::write(RespPOISum::LOG, [$ts=ts, $p=p,$prot=prot ,$counter=r$num]);
							}]);
