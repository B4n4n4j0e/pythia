...
event connection_state_remove(c: connection) &priority = 0
    ...
    SumStats::observe("resp_p",[],SumStats::Observation($str=cat(c$id$resp_p)));
...
event zeek_init() {
	...
	SumStats::create([	$name = "top k resp port",
						...
						s = topk_get_top(r$topk,10);
							...
							local resp_p: port = to_port(s[i]$str);
                            Log::write(RespPortTopK::LOG, [$ts=ts, $resp_p=resp_p,$proto=proto, $counter=topk_count(r$topk,s[i])]);
                            }
	...
	SumStats::create([	$name = "sum protocol types",
			            ...
			            Log::write(ProtoSum::LOG, [$ts=ts, $proto=key$str ,$counter=r$num]);
						}]);
