<template>
  <v-card>
    <v-data-table dense :headers="headers" :items="filters" hide-default-footer>
      <template v-slot:top>
        <v-toolbar flat>
                ...
                <v-container>
                  <v-row>
                    ...
                    <v-col class="d-flex" cols="8">
                      <v-text-field
                        ...
                        label="Filter*"
                        :rules="selectedOption != 'Text' ? numberRules: []"
                  ...
                  <v-row v-else-if="selectedOption == 'Port'">
                    <v-col cols="4">
                      <v-row
                        no-gutters
                        v-for="portOption in portOptions"
                        v-bind:key="portOption.text"
                        class="d-flex"
                      >
                        <v-checkbox
                  ...
                  <v-row v-else>
                    <v-col cols="8">
                      <v-row
                        no-gutters
                        v-for="compOption in compOptions"
                        v-bind:key="compOption.text"
                        class="d-flex"
                      >
                        <v-checkbox
                  ...
                  <v-row class="flex align-end" align-begin
                    >* Every space sets an additional filter</v-row
                  >
                </v-container>
              </v-card-text>
              <v-card-actions>
...
export default {
  ...
  data: () => ({
    ...
    numberRules: [
      (value) => {
        ...
        return pattern.test(value) || "Please use only numbers (e.g. 20 -2 0.2)"
      },
    ],
    ...
    textOptions: [
      { text: "Origin Host", isActive: false, filterName: "originHost" },
      { text: "Responder Host", isActive: false, filterName: "respHost" },
      { text: "Service", isActive: false, filterName: "service" },
      { text: "Protocol", isActive: false, filterName: "protocol" },
      { text: "Query", isActive: false, filterName: "query" },
      { text: "Query answer", isActive: false, filterName: "qAnswer" },
      { text: "Query type", isActive: false, filterName: "qType" },
      { text: "Error Code", isActive: false, filterName: "qError" },
      { text: "UID", isActive: false, filterName: "uID" },
    ],

    portOptions: [
      { text: "Responder Port", isActive: false, filterName: "respPort" },
      { text: "Origin Port", isActive: false, filterName: "originPort" },
    ],
    compOptions: [
      {
  ...
  computed: {
    filters() {
      var filterArray = [];
      var filters = this.$store.getters["summaryData/connectionFilters"];
      for (const [key, value] of Object.entries(filters)) {
        value.forEach((element) => {
          filterArray.push({ category: key, filter: element });
        });
      }
      return filterArray;
    },
  ...
  methods: {
    removeFilter(item) {
      this.$store.dispatch("summaryData/removeFilterByFilterName", item);
    },
    removeAllFilters() {
 ...
 var filters = this.filterText.split(" ");
      ...
      filters.forEach((filter) => {
        if (this.selectedOption == "Text") {
          ...
          if (!(this.patternNumber.test(this.filterText) && this.patternProtocol.test(this.portProtocolText))){
            return
          }
          ...
          this.compOptions.forEach((value) => {
            if (value.isActive) {
              // implement error message + regex https://stackoverflow.com/questions/11456850/split-a-string-by-commas-but-ignore-commas-within-double-quotes-using-javascript
